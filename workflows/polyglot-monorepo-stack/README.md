...
custom checkout token or per-app image build secrets. To read other private repositories during
checkout, use [`checkout-token-resources`](#checking-out-other-repositories) rather than a personal
access token.
...
The check, test and publish jobs request `contents: read`, `packages: read`, `actions: write` and
`id-token: write`, plus `contents: write`, `packages: write` and `pages: write` in the publishing
jobs. `actions: write` is what the Actions cache service requires to save entries, and
`id-token: write` is what the OIDC token exchange requires.
...
and private GitHub Package Registry dependencies are unavailable. Fork runs also have no OIDC token,
...
When set, this takes precedence over the `CHECKOUT_TOKEN` secret, which is deprecated. Pull requests
from forks have no OIDC token, so the exchange is skipped there and checkout falls back to
`CHECKOUT_TOKEN`, then the read-only `GITHUB_TOKEN`.
...
| `CHECKOUT_TOKEN`           | Access token to use for checkout.<br>**Default:** `Github Token`<br>**Note:** PAT requires an expiration date.<br>**Deprecated:** Prefer `checkout-token-resources`, which mints a short-lived scoped token via the token broker. Still honoured when `checkout-token-resources` is not set.                                                                                                                                                                                                                                                                                                 | No       |
